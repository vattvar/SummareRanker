...
class ModelMultitaskBinary(nn.Module):
    ...
    def __init__(self, pretrained_model, tokenizer, args):
        ...
        self.net = nn.Sequential(
                            nn.Linear(args.hidden_size, args.bottom_hidden_size),
                            nn.ReLU(),
                            nn.Linear(args.bottom_hidden_size, args.hidden_size)
                            ).to(args.device)
        ## ------------------------------
        # MoE
        ...
        self.towers = nn.ModuleList([L4_Tower(args.tower_hidden_size) for i in range(args.n_tasks)])
        #nn.ModuleList([MLPTower(args.tower_hidden_size) for i in range(args.n_tasks)])
        ...
        # training labels
        self.init_dict = {j: [] for j in range(self.args.n_tasks)}
        self.init_list = [[] for j in range(self.args.n_tasks)]
        self.original_training_labels = self.init_dict
        self.training_labels = self.init_dict
        self.training_scores = self.init_dict
        self.training_hits = self.init_dict

        # multi-summary evaluation
        self.multi_summary_pred_idx = self.init_dict
        self.multi_summary_preds = self.init_dict
    ...
    def forward(self, mode, text_and_summaries_ids, text_and_summaries_mask, scores):
        ...
        accuracy = list(np.zeros(self.args.n_tasks))
        #[0 for j in range(self.args.n_tasks)]
        rank = list(np.zeros(self.args.n_tasks))
        
        #[0 for j in range(self.args.n_tasks)]
        predictions_idx = [[] for j in range(self.args.n_tasks)]#self.init_list
        predictions = [[] for j in range(self.args.n_tasks)]#self.init_list
        total_predictions_idx = []
        overall_sums = []
        overall_predictions = []
        scores_individual = torch.zeros(self.args.n_tasks)
        for i in range(text_and_summaries_ids.shape[0]):
            ...
            encs = encs[:, 0, :]
            # shared bottom
            preds_i = self.net(encs) #self.fc2(self.relu(self.fc1(encs)))
            # MoE
            ...
            # print(torch.tensor([scores_i[j][total_prediction_idx].item() for j in range(self.args.n_tasks)]))
            scores_individual = scores_individual + torch.tensor([scores_i[j][total_prediction_idx].item() for j in range(self.args.n_tasks)])
        ...
        scores_individual = (scores_individual /text_and_summaries_ids.shape[0])
        outputs["r1"] = (scores_individual[0]).to(loss.device)
        outputs["r2"] = (scores_individual[1]).to(loss.device)
        outputs["rl"] = (scores_individual[2]).to(loss.device)
